...
spec_wal = do
    ...
    describe "Recover and Recover test" $ do
        before (removeFile "test2.wal" `catch` (\(_ :: IOException) -> return ())) $ do
            it "should recover a WAL file and continue writing" $ do
        ...
        after
            (\_ -> (removeFile "test2.wal" `catch` (\(_ :: IOException) -> return ())))
            $ do
                return ()
